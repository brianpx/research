/* ─────────────────────────────────────────────────────────
   Dropdown wiring (ACCESSIBILITY FIX: Initialize aria-expanded)
───────────────────────────────────────────────────────── */
document.addEventListener('click', e => {
    if (!e.target.closest('#typeDropdownWrap'))   {
...
(function init() {
    ...
    // Disclaimer modal (with focus trap + focus restore)
    const disclaimerModal = document.getElementById('disclaimerModal');
    let _disclaimerTriggerEl = null;

    function trapDisclaimerFocus(e) {
        if (e.key !== "Tab") return;
        const focusable = disclaimerModal.querySelectorAll('button:not([disabled]), a[href], [tabindex]:not([tabindex="-1"])');
    ...
    function openDisclaimer() {
        _disclaimerTriggerEl = document.activeElement;
        disclaimerModal.classList.remove('hidden');
        document.addEventListener("keydown", trapDisclaimerFocus);
        const first = disclaimerModal.querySelector('button');
        if (first) first.focus();
    }

    function closeDisclaimer() {
        document.removeEventListener("keydown", trapDisclaimerFocus);
        disclaimerModal.classList.add('hidden');
        if (_disclaimerTriggerEl) {
            _disclaimerTriggerEl.focus();
            _disclaimerTriggerEl = null;
        }
    }
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape" && !disclaimerModal.classList.contains("hidden")) closeDisclaimer();
    });

    // ACCESSIBILITY FIX: Initialize aria-expanded on dropdown triggers
